...
<body>
    ...
    <script>
        ...
        // Function to update navigation boxes (show only missed questions)
        function updateNavBoxes() {
            // Clear existing boxes 
            document.querySelectorAll('.question-nav-box').forEach(box => box.remove());

            // Convert unanswered questions to an array and sort it
            const unansweredArray = Array.from(unansweredQuestions).sort((a, b) => a - b);

            // Determine the end index based on the current start index
            const endIndex = Math.min(currentStartIndex + maxVisibleBoxes, unansweredArray.length);

            // Create navigation boxes only for unanswered questions in the current visible range
            for (let i = currentStartIndex; i < endIndex; i++) {
                const index = unansweredArray[i];
                const navBox = document.createElement('div');
                navBox.className = 'question-nav-box unanswered';
        ...
        // IntersectionObserver to track seen questions
